...
__global__ void op_elemwise_unary_kernel(OpFunc f, Tensor<T> t, Tensor<T> out)
{
    ...
    //cout<<i<<" "<<j<<endl;
    //////printf("(i,j) --> %d (j,i) --> %d",Index(t,i,j), f(Index(t,j,i)) );

    if(!IndexOutofBound(t, i, j)){
        //////printf("%d, %d --> %d \n ", i,j, Index(t,i,j));
        Index(out, i, j)= f(Index(t,i,j));
        //////printf("aaXX\n");
        //////printf("%f --> %f \n", Index(out, i, j),Index(t,i,j) );
        //std::cout<<Index(out, i, j)<<" --> "<<Index(t,i,j)<<std::endl;
    }
}
...
void op_elemwise_unary_gpu(OpFunc f, const Tensor<T> &t, Tensor<T> &out)
{
  ...
  op_elemwise_unary_kernel<<<numBlocks, threadsPerBlock>>>(f, t, out);
}
...
__global__ void op_elemwise_binary_w_bcast_kernel(OpFunc f, Tensor<AT> in1, Tensor<BT> in2, Tensor<OutT> out)
{
  //Lab-1: add your code here
   ...
   if(i<in1.h && j<in1.w && in1.h == in2.h && in1.w==in2.w){
   Index(out, i,j)  = f(Index(in1,i,j), Index(in2, i,j));
   }
   else if(i<in2.h && j<in1.w && in1.h == 1 ){
   Index(out, i,j)  = f(Index(in1,0,j), Index(in2, i,j));
   }
   else if(i<in1.h && j<in1.w && in2.h ==1){
   Index(out, i,j)  = f(Index(in1,i,j), Index(in2, 0,j));
   }
   else if(i<in1.h && j<in2.w && in1.w==1){
   Index(out, i,j)  = f(Index(in1,i,0), Index(in2, i,j));
   }
   else if(i<in1.h && j<in1.w && in2.w==1){
...
void op_elemwise_binary_w_bcast_gpu(OpFunc f, const Tensor<AT> &in1, const Tensor<BT> &in2, Tensor<OutT> &out)
{
   ...
   //delete assert(0) when you are done
    int32_t N1 = in1.h, M1= in1.w, N2 = in2.h, M2= in2.w;
    int32_t N = max(N1, N2), M= max(M1, M2);
    ...
    op_elemwise_binary_w_bcast_kernel<<<numBlocks, threadsPerBlock>>>(f, in1, in2, out);
    //////printf("binay passed");
    //assert(0);
...
{
    //std::cout<<t.h<<" "<<dt.h<<" "<<t.w<<" "<<dt.w<<std::endl;
    assert(out.h == t.h && out.w == t.w);
...
template <typename T>
bool op_allclose_2(const Tensor<T> &at, Tensor<T> &bt)
{
    using namespace std;
    ////printf("Test Anum -1\n");
    if (at.h != bt.h || at.w != bt.w)
